...
main ( int argc, char * argv[] )
{
    ...
    f = fopen("ex22.txt","w+");
    fprintf(f,"Test \n");
    for(int h = 10; h< 31; h++){                                            //loop through data sizes
        int optMemorySize = (1<<h);
        for(int k = 1; k<= 1024; k=k*2){                                            //loop through data sizes
            //long bytes = (1<<j) / sizeof(float);
            ...
            for (int i = 0; i < optMemorySize/sizeof(int); i++) {
                h_memoryA[i] = i;
            };
            ...
            // Global Memory Tests
            //
            printf("Starting Kernel to copy: \n");
            kernelTimer.start();
            ...
            cudaMemcpy(h_memoryB, d_memoryB, optMemorySize, cudaMemcpyDeviceToHost);
            for(int i = 0; i<optMemorySize/sizeof(int); i++){
                if (h_memoryB[i] != h_memoryA[i]) std::cout<<h_memoryA[i]<<" "<<h_memoryB[i]<<std::endl;
                break; 
            }
            printf("Kernel finished copying \n");
            //
            // Check for Errors
            //
            ...
            if ( !optUsePinnedMemory ) { // Pageable
                std::cout << "***" << " Freeing pageable memory" << std::endl;
                free(h_memoryA);
                free(h_memoryB);
            } else { // Pinned
                std::cout << "***" << " Freeing pinned memory" << std::endl;
                // Allocation of pinned host memory
                cudaFree(h_memoryA);
                cudaFree(h_memoryB);
            }
            ...
            if ( chCommandLineGetBool ( "global-coalesced", argc, argv ) ) {
                ...
                fprintf(f,"%i \t %i \t %f \n",optMemorySize,optGridSize, optMemorySize / kernelTimer.getTime(optNumIterations) / (1E09));
            }
